...
The Riemann hypothesis, in the exact form in which mathlib states it
(\texttt{RiemannHypothesis : Prop}, unproved in mathlib):

\begin{quote}
For every $s \in \mathbb{C}$ with $\zeta(s) = 0$ and $0 < \mathrm{Re}(s) < 1$,
we have $\mathrm{Re}(s) = 1/2$.
\end{quote}

Here $\zeta$ is the Riemann zeta function as defined in mathlib
(\texttt{Mathlib.NumberTheory.LSeries.RiemannZeta}); ``nontrivial zero''
means a zero in the critical strip $0 < \mathrm{Re}(s) < 1$. The two
trivial zeros at negative even integers are outside the statement, as in
the standard formulation \cite{edwards1974riemann,hardy2008introduction}.

\subsection{Axioms}

Every theorem in this manuscript depends only on the three standard Lean
logic axioms plus mathlib's definitions and theorems.
\verb|#print axioms| on representative theorems yields exactly
\texttt{[propext, Classical.choice, Quot.sound]}. No new axioms, no
\texttt{sorry}, no \texttt{axiom} declarations were added. The full build
is 3631 jobs against Lean 4.32.2 / mathlib v4.32.2.

\subsection{The link chain C011--C025}

Fifteen claims, each with its Lean file, its machine-checked theorem
names, and its status. Status legend:
\textcolor{proved}{\textbf{LP}} = machine-checked in Lean (no sorry);
\textcolor{classical}{\textbf{CK}} = content classical, machine-checked as
a restatement; \textcolor{conj}{\textbf{CONJ}} = conjecture, not
machine-checked.
...
The machine-checked dependency order (reading order of the files):
...
Toolchain Lean 4.32.2; mathlib v4.32.2; full build 3631 jobs, no
\texttt{sorry}. Source files: \texttt{Formal/ZeroRelative/ComplexAxis.lean}
(2211 lines, 128 theorems), \texttt{Formal/ZeroRelative/
PrimeDriftPositions.lean} (140 lines, 6 theorems),
\texttt{Formal/ZeroRelative/ZetaEulerProduct.lean} (160 lines, 7
theorems). Claims ledger with per-claim SHA-256 publication: C011--C025,
all \texttt{status: PROVED}, \texttt{novelty\_status: KNOWN}
(restatements of classical mathematics). An anonymous mirror of this
manuscript and the Lean sources is archived alongside the claims
(\texttt{papers/cpp2027\_riemann\_verification\_map/}).

\subsection{The status of the map}

Fifteen links are machine-checked (\textcolor{proved}{LP} /
\textcolor{classical}{CK}). The Riemann hypothesis itself is one
additional link, shown \textcolor{conj}{\textbf{CONJ}} in the chain: no
theorem in this manuscript asserts that every nontrivial zero of $\zeta$
lies on the critical line. That assertion is the target of the map, not
one of its edges. Section~\ref{sec:gap} states precisely what the missing
link is and what a proof of it would have to show.

\section{Why a verification map}

A submission by an independent researcher faces a specific problem that a
...
exact form of the standard conjecture; it lists the axioms; it names every
link of the chain with its machine-checked theorem; it draws the
...
credibility mechanism we adopt, and the remainder of this manuscript is
the map rendered in full.
...
edge is a proof object whose correctness is decidable --- and the map
keeps the discipline even where the chain ends.

\section{The standard statement}

We fix the statement to which every link refers. Mathlib defines the
Riemann zeta function $\zeta$ (as the meromorphic continuation of
$\sum n^{-s}$) and declares the Riemann hypothesis as a proposition
\texttt{RiemannHypothesis : Prop}, left unproved. In this manuscript the
hypothesis is the statement: every complex zero $s$ of $\zeta$ with
$0 < \mathrm{Re}(s) < 1$ satisfies $\mathrm{Re}(s) = 1/2$. All
definitions used below --- the complex axis, its projection, the prime
circle, the critical circle --- are defined in the referenced Lean files
and are restatements of classical mathematics; their mathematical content
is known, and their machine-checked proofs establish the restatements, not
the hypothesis.

Two classical facts frame the statement \cite{edwards1974riemann}:
(i) the functional equation $s \mapsto 1-s$ relates $\zeta(s)$ to
$\zeta(1-s)$, so the zeros are symmetric about the critical line; (ii)
$\zeta$ has no zeros on $\mathrm{Re}(s) \ge 1$ (a consequence of the Euler
product), and the trivial zeros at $-2, -4, \ldots$ are known. The
hypothesis concerns the remaining, nontrivial zeros in the strip. The
chain below formalizes the geometric shadow of these facts: the
...
correction to the Riemann--Siegel formula. The observation: the zeros
form a curve, and the curve is the image of the critical line under a
folding (inversion-like) map. The numerical record is observational; it
is not part of any machine-checked statement.
...
have no construction method (all other numbers have many); irrationals are
the remainder of a higher-dimensional structure after projection lost part
of its carrying structure; and the pair $1$ vs $1/a + 1/b$ (translation vs
inversion) leads to the conjecture that the basepoint has drifted --- the
origin moves on the circle of radius 1 around 1, and the drifted
basepoint's successor iteration, projected back to the number line, lands
on primes. This document is the intuition source of the chain; the chain
itself is the machine-checked rendering of that intuition into classical
mathematics.
...
step $d$. The observations formalized in
\texttt{PrimeDriftPositions.lean} are six theorems. The successor of the
basepoint is the basepoint plus the step
(\texttt{drift\_vector\_is\_successor}); the $n$-th successor lies at
$e + (n+1)d$ (\texttt{successor\_positions\_in\_drift\_frame}); a
multiple $n \cdot p$ of a prime $p$ is prime exactly when $n = 1$
(\texttt{prime\_positions\_on\_drift\_axis}) --- primes sit one successor
from the basepoint and nowhere else; the inversion-dual position
$1/(1/a + 1/b) = ab/(a+b)$ (\texttt{inversion\_dual\_position}) applied
to a prime puts $p/2$ at a non-integer position for every odd prime
(\texttt{prime\_inversion\_misses\_integers}); and an irrational drift
step produces an axis with no integer points at all
(\texttt{irrational\_drift\_axis\_no\_int\_points}).

Why these six statements? They test the intuition that ``prime-ness is a
positional property'': if primes are positions on an axis, then a change
of drift step moves everything except the basepoint-adjacent slot, and
the primality of $n \cdot p$ is destroyed exactly when $n > 1$. The
...
$\mathbb{R}$; the proofs are direct.
...
The mathematics here is the elementary algebra of the complex numbers
viewed as $\mathbb{R}^2$ with a preferred projection; every statement is
a restatement of facts about $\mathbb{C}$ that mathlib proves. What the
formalization adds is the precise statement of the ``origin illusion'': no
projected observation can distinguish the real axis from any other axis
of the same kind, so any claim about the specialness of the real axis is
not an observation but a choice. This will matter in
...
\texttt{zeta\_two\_terms}. This anchors the series that the Euler product
below equals, and it fixes the notation used by the conditional
...
ordinary partial sum of the zeta series.
...
$n + J$ and projects. Its significance is that the projected image
depends on $n$ through the rational function $n/(n^2+1)$, which is
bounded and small for large $n$ --- the double drift compresses the axis,
and the compression is the first geometric expression of ``the primes are
reachable by drift''.
...
The chain C014--C017 establishes the bridge between the number-theoretic
...
Two remarks keep the boundary honest. First, each of these statements is a
pure geometric identity; none refers to the analytic value of $\zeta$ at
any point. Second, the special points $0$ and $2$ have a geometric
meaning: $0$ is the image of the point at infinity under inversion
(\texttt{recip\_vanishes\_at\_infinity}) and $2$ is the image of
$\frac12$ (\texttt{halfBasepoint\_recip} and the real-axis intersection
statement). The excluded points of
\texttt{criticalCircle\_subset\_zeroSet\_image} are exactly the images of
the two points of the critical line that are not hypothetical zero
positions.
...
a zero on the critical line is a point of the critical circle.

\section{The gap}\label{sec:gap}

The verification map has one open edge. The machine-checked chain proves:
(i) the critical line is the circle $\{w : \|w - 1\| = 1\}$ under
inversion (C019--C022); (ii) the hypothetical zero positions sit on that
circle (C021--C022); (iii) the Euler product converges exactly on
$\mathrm{Re}(s) > 1$ and $\zeta$ has no zeros on $\mathrm{Re}(s) \ge 1$
(C025). It does not prove: that every nontrivial zero of $\zeta$ lies on
the critical line.

To close the gap a proof would have to show that no zero of $\zeta$
exists off the line $\mathrm{Re}(s) = 1/2$ within the strip
$0 < \mathrm{Re}(s) < 1$. The geometry of this manuscript neither assumes
nor implies that statement. Two structural remarks make the gap precise.
...
\textcolor{conj}{CONJ} node is the final edge --- the map's purpose is to
make exactly this edge visible to the referee.
...
circle other than the critical circle. The question ``does $\zeta$ have a
zero off the line?'' is equivalent, within this map, to the question
...
evidence, not proof; it is recorded here because it is the trail's
starting point and because the verification map separates it cleanly from
the machine-checked chain. External numerical verification of the first
$10^{13}$ zeros (real parts equal to $1/2$) is likewise observational and
is not part of any machine-checked statement.

\section{Honest boundary}

Every claim in this manuscript is labeled \texttt{novelty\_status: KNOWN}:
each is a restatement of classical mathematics (circle inversion, Gaussian
unique factorization, Fermat's two-square theorem, the Euler product,
elementary projection algebra) with a machine-checked proof. No claim is
made of proving the Riemann hypothesis; mathlib's
...
This boundary is not an apology; it is the method. The credibility of the
manuscript does not rest on the strength of its claims but on the
exactness of their labeling. A referee who disagrees with any label can
verify it in minutes; a referee who agrees with all labels has a complete
picture of what is proved, what is classical, and what is open.

\section{Related work}

The formalization of the Riemann zeta function in mathlib (as an L-series
with meromorphic continuation) provides the analytic base
...
correctness'' is the standard of interactive theorem proving, applied here
to the structure of a submission rather than to its individual steps. The
novelty of the present manuscript is not mathematical --- every
machine-checked statement is known --- but organizational: a template for
independent researchers in which the manuscript's trustworthiness is
decidable from the manuscript itself.

\section{Conclusion}

A manuscript organized as a verification map makes its own trust decision
mechanical. Fifteen links of the Riemann-direction chain are
