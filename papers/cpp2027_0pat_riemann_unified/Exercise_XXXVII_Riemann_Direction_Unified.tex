...
machine-checked development of one hundred and fifty-six declarations
(one hundred and twenty-four theorems and lemmas and thirty-two
definitions), all formalized in Lean 4 + mathlib from known
mathematics alone. The
...
flip-to-zero theorem). All one hundred and fifty-six declarations (one
hundred and twenty-four theorems and lemmas and thirty-two definitions)
are machine-checked in Lean 4 + mathlib from known mathematics alone,
...
and the intermediate-value direction of the flip count: a sign change
of $Z$ between $a$ and $b$ forces a zero of $\zeta$ on the critical
line (\texttt{hardyZ\_zero\_between}). The Riemann hypothesis is
equivalent to all zeros of $Z$ being real; that statement is not
claimed. Proving that $Z$ has no non-real zeros requires the
converse direction of the zero counting --- that every critical-line
zero is counted by a flip and that the total count matches the
argument-principle count $N(T)$ --- which is beyond the
machine-checked content of this exercise. The Riemann hypothesis
remains open.

\section*{Artifact}

The artifact contains \texttt{proof\_unified.lean} (Lean 4.32.2,
mathlib v4.32.2): one hundred and fifty-six declarations (one hundred
and twenty-four theorems and lemmas and thirty-two definitions), zero
errors, zero \texttt{sorry}; the file is the self-contained unification
