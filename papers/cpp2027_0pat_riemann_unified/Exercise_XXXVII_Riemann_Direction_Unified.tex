...
machine-checked development of one hundred and five theorems, all
...
All one hundred and ten theorems are machine-checked in Lean 4 +
mathlib from known mathematics alone, with zero errors and zero
\texttt{sorry}; the fifth section's functional equation and the
completed zeta facts are cited from mathlib rather than proved anew.
...
divergence factor $n^{-1/2}$. The phase-base-point projection of
numerical observation (the Hardy $Z$ function, which projects the
values of the zeta function on the critical line onto the real axis)
is recorded in the artifact as numerical evidence, not as a proof:
establishing that $Z$ has no non-real zeros requires the analytic
theory of zero counting and of sign changes, which is beyond the
...
v4.32.2): one hundred and ten theorems, zero errors, zero
\texttt{sorry}; and \texttt{observation.md}, a record of twelve
numerical observations (critical strip, zero locations, base-point
shift, phase projection, and exhaustive radius searches). Build:
