...
machine-checked development of two hundred and forty-eight declarations
(two hundred theorems and lemmas and forty-eight definitions), all
formalized in Lean 4 + mathlib from known mathematics alone. The
...
(\texttt{hardyZ\_conj}, \texttt{hardyZ\_im\_zero}). The proof is a
conjugation-reflection symmetry: $\overline{Z} = Z$ (the conjugate
of each factor of $Z$ is computed in place, and the unit modulus of
$\chi$ closes the conjugation), rather than an algebraic argument on
$Z^2$; the file \texttt{HardyZ.lean} imports Mathlib only.

\paragraph{Flip $=$ zero: sign changes of $Z$ force critical-line
zeros (hardyZ\_zero\_between).} The real-valuedness of $Z$ is the
first missing ring of the flip count. The second ring is the
intermediate value theorem. Machine-checked: $Z(t) = 0$ if and only
if $\zeta(1/2+it) = 0$ (the multiplier is never zero on the line,
hence the power $\chi^{-1/2}$ is never zero); $Z$ is continuous
($\chi$ is continuous on the critical line --- the power factor
$2\cdot(2\pi)^{s-1}$ has its base $2\pi$ in the slit plane, the
Gamma factor is continuous off the negative integers, and $\sin$ is
continuous; $\zeta$ is differentiable off $s=1$; and the power
$c\mapsto c^{-1/2}$ is continuous at $c=\chi(1/2+it)$ because the
multiplier lies in the slit plane); and, for $a<b$,
\[
\operatorname{Re}Z(a)<0<\operatorname{Re}Z(b)
\implies \exists\,t\in(a,b),\ Z(t)=0
\]
(\texttt{hardyZ\_zero\_between}, \texttt{intermediate\_value\_Icc}).
A flip --- a sign change of $Z$ across the critical line --- therefore
always records a zero of $\zeta$ on the critical line: every flip
counts toward $N_0(T)$.

\section{Splitting the zeta function: even/odd parts and the base point $i$}

Conjugation symmetry projects onto the real and imaginary parts
(machine-checked):

\paragraph{Re/Im split.} In the critical strip,
$\operatorname{Re}\zeta(s) = \operatorname{Re}\zeta(\overline{s})$
(zeta\_re\_conj\_symm: the real part is even) and
$\operatorname{Im}\zeta(s) = -\operatorname{Im}\zeta(\overline{s})$
(zeta\_im\_conj\_antisymm: the imaginary part is odd). On the
critical line this gives the even/odd structure
$\operatorname{Re}\zeta(1/2+it)$ even and
$\operatorname{Im}\zeta(1/2+it)$ odd in $t$. A zero is the common
zero of both parts: $\zeta(z) = 0 \iff \operatorname{Re}\zeta(z) = 0
\land \operatorname{Im}\zeta(z) = 0$ (zeta\_eq\_zero\_iff\_re\_im).

\paragraph{p-adic split (base point $p$).} For any base point
$p \neq 0$ the zeta function splits algebraically as
$\zeta = O_p + E_p$ with $O_p = (1-p^{-s})\zeta$ and
$E_p = p^{-s}\zeta$ (zeta\_eq\_odd\_add\_even); a zero is the common
zero of both parts (zeta\_eq\_zero\_iff\_p\_split). For $p = 2$ this
is the parity split into the non-multiples and the multiples of $2$.
The extra zeros of the odd part, those with $\zeta(s) \neq 0$, lie on
the imaginary axis: $1-2^{-s} = 0 \iff s\ln 2 \in 2\pi i\mathbb{Z}$
(odd\_part\_extra\_zero\_on\_imag\_axis); they never enter the
critical strip, so the intersections of the two parts are exactly the
zeros of $\zeta$.

\paragraph{The zero region.} The prime/Euler-series viewpoint closes
the region: $\zeta(s) \neq 0$ for $\operatorname{Re}s \geq 1$
(mathlib's nonvanishing theory of Dirichlet $L$-series), and the
functional equation mirrors this to $\operatorname{Re}s \leq 0$
(riemannZeta\_ne\_zero\_of\_re\_lt\_zero and
riemannZeta\_ne\_zero\_of\_re\_eq\_zero), so every non-trivial zero
lies in the critical strip $0 < \operatorname{Re}s < 1$
(nontrivial\_zero\_in\_critical\_strip).

\paragraph{The base point $i$ and the T-coordinate.} The observation
frame of this exercise places the origin of the complex plane at
$i$, the tick $1$ of the fake real axis being $i^i = e^{-\pi/2}$ and
the half-tick $1/2$ being $i^{i/2} = e^{-\pi/4}$. The critical line
is then the perpendicular bisector of $i$ and $1+i$:
$\operatorname{Re}z = 1/2 \iff |z-i| = |z-(1+i)|$
(critical\_line\_equidistant\_basepoint\_i). Choosing the inversion
centre at the base point $i$, the map
$T(z) = 1/(z-i) - 1$ (inversion plus translation $-1$ cancelling the
centre of the image circle) sends the critical line to the unit
circle:
\begin{equation*}
  \operatorname{Re}z = 1/2 \iff |T(z)| = 1
\end{equation*}
(recip\_basepoint\_i\_on\_unit\_circle). The inversion centre at a
point of the imaginary axis leaves the image circle centred at $1$
(the line $\operatorname{Re}z = 1/2$ is translation-invariant in the
imaginary direction); the translation $-1$ then moves the centre to
the origin. One base point, $i$, resolves both the circle image of
the critical line and the cancellation of the translation/inversion
pair.
...
observation, the iteration of $i$, the completed zeta structure, and
the Hardy $Z$ function (real-valuedness and the intermediate-value
flip-to-zero theorem). All two hundred and forty-eight declarations (two
hundred theorems and lemmas and forty-eight definitions) are
machine-checked in Lean 4 + mathlib from known mathematics alone,
with zero errors and zero \texttt{sorry}; the functional equation and
the completed zeta facts are cited from mathlib rather than proved anew.
...
imaginary axis, the trigonometric origin of the trivial zeros, the
observation that the critical line is exactly the locus of the
divergence factor $n^{-1/2}$, the machine-checked fact that the
function $Z(t) = \chi(1/2+it)^{-1/2}\zeta(1/2+it)$ is real-valued,
and the flip-count chain: a sign change of $Z$ forces a critical-line
zero (\texttt{hardyZ\_zero\_between}, intermediate value); the flip
count equals the $\chi$ winding number minus an integer layer
(\texttt{flip\_count\_from\_theta\_lifts}); the integer-layer
structure of $S(T)$ (\texttt{zeta\_lift\_half\_chi\_lift\_pi\_int});
the finite--infinite bridge of the error sequence
(\texttt{poly\_error\_ratio}: $e(2n)/e(n) = 2^{1-s}$, measuring
once predicts all); and the rectangle closure
(\texttt{completedZeta$\_0$\_log\_reflection},
\texttt{zeta\_zero\_iff\_completedZeta$\_0$\_one\_over\_sum})
with its boundary estimates. The Riemann hypothesis is equivalent to
$N_0(T) = N(T)$ for all $T$, i.e. to the absence of off-line zeros;
that statement is not claimed, and the off-line orbit structure
(\texttt{zero\_orbit\_counting}: four-point orbits off the line)
remains the honest boundary. The Riemann hypothesis remains open.

\section*{Artifact}

The artifact contains \texttt{proof\_unified.lean} (Lean 4.32.2,
mathlib v4.32.2): two hundred and forty-eight declarations (two
hundred theorems and lemmas and forty-eight definitions), zero errors,
zero \texttt{sorry}; the file is the self-contained unification
of the iso development blocks with the Hardy $Z$ function (imports
Mathlib only); and \texttt{observation.md}, a record of thirty
