...
% 读者向导框 (视觉引导: 邀请式语言, 描述结构而非指路, 避免规训感)
\newtcolorbox{readerguide}{colback=gray!6,colframe=blue!45!black,
  boxrule=0.8pt,arc=2pt,title={\small\textbf{The paper at a glance}},
  fonttitle=\normalsize,left=8pt,right=8pt,top=4pt,bottom=4pt}
...
\begin{abstract}
We present a machine-verified development of the Riemann direction,
organized by the symmetries that force it: reflection
$s\mapsto 1-s$ across the critical line, conjugation, and the pairing
of the odd and even parts of the series. The development covers the
...
pairing (iso~41), a single inequality replacing an entire classical
machinery. The development is a chain of 237 verified Lean~4
...
item being a theorem of the listed layer:
\begin{itemize}
...
continuity of $\zt$ on $(0,1)$ by the odd/even pairing (iso~41,
machine-verified) and the sign $\zt(x)<0$, closing the zero-free bottom
...
\begin{tikzpicture}[
  ...
  \node[core, right=4mm of L12] (L13) {\small L13: $(0,1)$};
  % 箭头
  \draw[edge] (bg.south) -- (L8.north);
  \draw[edge] (fe.south) -- (L9.north);
  \draw[edge] (L8) -- (L9) -- (L10) -- (L11) -- (L12) -- (L13);
  % 标注
  \node[lab, above=1.2mm of L13] {iso 41};
  \node[lab, below=1.2mm of bg] {self-contained background of the representation};
  \node[lab, below=1.2mm of fe] {bridges the series and the functional equation};
  \node[lab, below=1.2mm of L8, align=center] {analytic core:\\ each box is a machine-verified layer};
\end{tikzpicture}
\caption{Structure of the development. The analytic core of the Riemann
direction is the chain L8--L13 (boxed); the 0pat representation
L1--L5 and the series/functional-equation layer L6--L7 are the
self-contained background from which the core is assembled. The
odd/even pairing of iso~41 is machine-verified independently and
closes Layer~13.}
\label{fig:map}
\end{figure}

\begin{readerguide}
The paper is organized as a chain of thirteen machine-verified layers.
The analytic core --- the part connecting the zeros of $\zt$ to the
argument principle --- is the chain L8--L13
(\S\ref{sec:L8}--\S\ref{sec:kernel}): the false-real-axis framework, the
critical-line phase layer, the covering-space phase locking, the
predictor residuals, the rectangle closure, and the $(0,1)$-segment.
The representation layers L1--L5 (\S\ref{sec:L1}--\S\ref{sec:L5}) are
self-contained background, and the framework discussion
(\S\ref{sec:discussion}) is context; neither is needed for the analytic
chain. Readers of the classical theory of $\zt$ will find their natural
entry points in \S\ref{sec:L8} and \S\ref{sec:L12}--\S\ref{sec:kernel};
readers interested in machine verification will find the full
declaration inventory in \S\ref{sec:verification} and Appendix~A.
\end{readerguide}
...
This layer records how zeros of $\zt$ behave under the reflection
$s\mapsto 1-s$; its reflected-pair theorem seeds the orbit structure of
Layer~9.
...
five intersection theorems of Layer~3. This is background of the
representation; the analytic core of the development begins at
Layer~8.}
\label{fig:circles}
\end{figure}
...
This layer fixes the geometry of the representation: the two basepoint
circles of Layer~1 and their intersections (Figure~\ref{fig:circles}).
It is self-contained background of the analytic core.
...
This layer records the second geometric symmetry of the representation
(transposition of the axes) and the uniqueness of the lattice points it
determines; like Layer~3 it is background.
...
This layer is the algebraic seed of the odd/even splitting used in the
analytic core: it records how the imaginary part of the representation
leaks into the real part under multiplication.
...
This layer explains why the zeta series cannot be split naively into its
odd and even parts: each part diverges, and only the period-pair
cancellation restores convergence --- the analytic fact behind the
odd/even pairing of Layer~13.
...
What could survive the divergence of both parts --- and what would it
mean for the interval $(0,1)$, where the series does not converge at
all?

\subsection{Layer 7: zeros and the functional equation}
\label{sec:L7}

This layer records the zeros of $\zt$ (trivial zeros, the phase of $i$)
and the functional equation in its completed form: the passage from the
meromorphic $\zt$ to the entire $\Lam_0$ used by the rectangle argument
of Layer~12.
...
The identity $i^i=e^{-\pi/2}$ is the Euler-style surprise of this layer:
the imaginary power of the imaginary unit is a real number in $(0,1)$ ---
a pure constant of the phase structure.
...
(\texttt{hardyZ\_real}, Layer~8); the phase structure of $\Xchi$ itself
is the subject of this layer.
...
their deterministic prediction is the subject of Layer~11. They are
integers --- but which integers, and what exactly do they count? The
question is answered only when the rectangle is closed.
...
functional equation. This is the closure argument of the analytic
core: counting the zeros of $\zt$ inside the rectangle is counting the
flips of its phase across the critical line.}
\label{fig:rect}
\end{figure}
...
What can survive there is the question left open by Layer~6.
...
What appeared in Layer~6 as an obstruction --- the divergence of each part
--- is here the mechanism itself.
...
In the economy of the whole development: the Riemann direction is
forced by two symmetries (reflection and conjugation) and closed by one
pairing; every one of the 237 declarations is a machine-verified step
