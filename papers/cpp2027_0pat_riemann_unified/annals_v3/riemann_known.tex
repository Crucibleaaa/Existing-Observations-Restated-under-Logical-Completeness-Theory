...
for the multiplier of the functional equation, the rectangle phase-lift
bookkeeping with its boundary estimates, conditionally equivalent to the
strip zero count (Theorem~6.4), the doubling and monotone
structure of the power-law error prototype, and the negativity of the zeta
function on the real interval $(0,1)$ --- the latter closed in this revision
by the odd/even pairing (iso~41). The development is a chain of 235 verified
Lean~4 declarations organized in thirteen layers, from a self-contained
algebra of complex axes (the basepoint-relative 0pat representation) through
the geometry of the prime and critical circles, the false-real-axis framework
(no zeros for $\Re s<0$, real-valuedness on the real axis), the
covering-space phase locking of the argument (the lifted $\theta_\Xchi$, the
flip counts), the power-law error prototype, and the rectangle bookkeeping with its
boundary estimates. Methodology statement:
...
Dirichlet series and has no zeros for $\Re s<0$ by the functional equation
(the trivial zeros $-2,-4,\dots$ are the only zeros on the negative real
axis). On the real interval $(0,1)$ the series diverges, and the classical
fact
\begin{equation}
\label{eq:main}
...
theorems connecting the zeros of $\zt$ to the phase of the multiplier
$\Xchi$, the rectangle phase-lift bookkeeping with its boundary estimates
(conditionally equivalent to the strip zero count, Theorem~6.4), and the
doubling and monotone structure of the power-law error prototype
(Layer~11, \S\ref{sec:L11}).

The full development is a chain of 235 Lean~4 declarations organized in
...
L11 & Power-law error prototype (doubling and monotone structure) \\
L12 & Rectangle phase-lift bookkeeping and boundary estimates (conditional equivalence to the zero count) \\
...
algebra over the complex numbers, Dirichlet series, integration by parts, the
Weierstrass theorem on locally uniform limits of holomorphic functions, and
the identity theorem. The reason is epistemic calibration: the author does
...
(in version 1: the Euler--Maclaurin continuation Blocks C--G of
\S\ref{sec:kernel}). In this revision the boundary is closed: the odd/even
...
files and the hash ledger (\texttt{iso\_hashes.md}) are archived at DOI
10.5281/zenodo.22038154 (version 1) and 10.5281/zenodo.22039506 (version 2,
containing Layer 13 Blocks A--B).
...
ZCode environment) performed the mechanical proof expansion and the writing
...
\]
and the library identity \texttt{zeta\_limit\_aux1} (for $1<s$) is
\[
...
basepoint circles are distinguished:
...
the line occur in pairs. This is the first appearance of the reflection
...
six intersection theorems of Layer~3.}
\label{fig:circles}
\end{figure}
...
the intersection points with the axes and with each other are located by the
six intersection theorems (\texttt{imaginary\_radius\_circle},
\texttt{real\_radius\_circle}, \texttt{imagAxis\_inter\_criticalCircle},
...
period-pair cancellation restores convergence. This is the analytic content
...
where the two unpaired sides diverge.
...
(\texttt{xi\_symmetry\_zero}, \texttt{xi\_symmetry}) and entire
...
The passage from the meromorphic $\zt$ to the entire $\Lam_0$ is the
...
(\texttt{chi\_conj}, \texttt{conj\_completedRiemannZeta\_zero}), and $\zt$
...
\label{thm:position}
The critical line is affine (\texttt{affine\_image\_critical\_line\_is\_line}),
...
(\begin{sloppypar}\texttt{critical\_line\_positional}, \texttt{orbit\_center\_half},
...
\label{thm:phaseid}
On the critical line, at points where $\zt(1/2+it)\neq0$ (the first
identity modulo $2\pi$),
\begin{equation}
\label{eq:phase}
2\arg\zt(1/2+it)=\arg\Xchi(1/2+it),\qquad
u(1/2+it)^2=\Xchi(1/2+it),
\qquad u=\zt/|\zt|,
\end{equation}
...
relation $u^2=\Xchi$ used in Layer~10: the unit phase $u$ of $\zt$ squares
to the multiplier, which is why the flip of $u$ across the line is governed
by the phase of $\Xchi$.
...
phase: they count the winding of $u$ along the rectangle (Layer~12).

\section{Layer 11: the power-law error prototype}
\label{sec:L11}

\begin{theorem}[Power-law error prototype]
\label{thm:residual}
For the power-law prototype $e_s(n)=n^{1-s}/(s-1)$ --- the power-law term
that governs the Euler--Maclaurin remainder (\eqref{eq:kernel}) --- the
machine-verified statements are (\texttt{polyError},
\texttt{poly\_error\_ratio}, \texttt{error\_seq\_predicts},
\texttt{poly\_error\_monotone}, \texttt{error\_precision\_bound}), all for
$n\ge1$:
\begin{equation}
\label{eq:polyerror}
e_s(2n)=2^{1-s}e_s(n)\quad(s\neq1),\qquad
e_s(2n)<e_s(n)\quad(s>1),\qquad
n\ge(1/\varepsilon)^{1/(s-1)}\implies e_s(n)\le\frac{\varepsilon}{s-1}
\quad(s>1,\ \varepsilon>0).
\end{equation}
\end{theorem}

That is: the prototype decays by the fixed ratio $2^{1-s}$ under doubling,
is monotone for $s>1$, and reaches precision $\varepsilon$ at the explicit
threshold $n=(1/\varepsilon)^{1/(s-1)}$. These are elementary power-law
identities, machine-verified from the definition of $e_s$ alone; no
property of the zeta function enters the proof. The prototype is recorded
here because the same power law $n^{1-s}/(s-1)$ is the term that governs
the Euler--Maclaurin remainder of \S\ref{sec:kernel}.

\emph{What is not claimed.} No connection between this prototype and the
counting remainder $S(T)=\frac1\pi\arg\zt(1/2+iT)$ of the
Riemann--von~Mangoldt formula is established in this paper, and none is
machine-verified: the deterministic prediction of $S(T)$ by a power-law
error remains an author's direction (the author's proposals, item~9). The
classical bound $S(T)=O(\log T)$ is KNOWN mathematics
\cite{titchmarsh}; it is not machine-verified here, and the present layer
does not imply it.

\section{Layer 12: the rectangle --- phase bookkeeping and the conditional zero count}
\label{sec:L12}
...
A \emph{flip} is a sign change of $\Re\zt$ along the critical line
(equivalently of the real-valued Hardy $Z$-function of
Theorem~\ref{thm:position}); the \emph{flip count} is the number of such
sign changes. The \emph{winding number} of $\Xchi$ along a path is the
total change of its lifted argument divided by $2\pi$.

Two counts must be kept distinct. Let $N_0(T)$ be the number of zeros of
$\zt$ on the critical-line segment $[1/2,1/2+iT]$, counted with
multiplicity, and let $N(T)$ be the number of zeros of $\zt$ in the strip
rectangle $[0,1]\times[0,T]$. Each count satisfies a classical explicit
formula (Backlund; Riemann--von~Mangoldt; \cite{titchmarsh}):
\begin{equation}
\label{eq:rvm}
N_0(T)=\frac1\pi\theta_\Xchi(T)+1+S(T),
\qquad
N(T)=\frac1\pi\theta_\Xchi(T)+1+S(T),
\end{equation}
where $\theta_\Xchi(T)$ is the lifted argument of $\Xchi$ along the
segment and $S(T)$ is the counting remainder of \S\ref{sec:relation}. The
two equalities of \eqref{eq:rvm} are separate classical theorems, each
KNOWN and not machine-verified in this paper. Their coincidence
$N_0(T)=N(T)$ is equivalent to the Riemann hypothesis --- precisely the
conditional equivalence of Theorem~\ref{thm:rheq}, which is
machine-verified. The present layer machine-verifies the phase-lift
bookkeeping that the two classical counts have in common, together with
the boundary estimates of \S\ref{sec:boundary}.

\subsection{The integer-layer bridges}
\label{sec:bridge}

\begin{theorem}[Phase bookkeeping on a zero-free segment]
\label{thm:flipcount}
On a segment of the critical line free of zeros of $\zt$, the lifted
phases of Layer~10 satisfy the alignment
\[
2\,\Delta\theta_{\zt}=\Delta\theta_{\Xchi}+2\pi i\,m\qquad(m\in\Z),
\]
where $\Delta$ denotes the total change along the segment
...
\]
(\texttt{completedZeta\_zero\_log\_reflection}), and in the strip
\[
\zt(s)=0\iff\Lam_0(s)=\frac{1}{s}+\frac{1}{1-s}
\]
(\texttt{zeta\_zero\_iff\_completedZeta\_zero\_one\_over\_sum}).
\end{theorem}
...
\emph{Proof sketch of Theorem~\ref{thm:flipcount}.} On the segment the
identity $u^2=\Xchi$ of \eqref{eq:phase} lifts to the pointwise layer
statement $\theta_{\zt}-\theta_{\Xchi}/2\in\pi i\Z$ (Layer~10), and
taking the total change along the segment gives the displayed alignment;
the machine-verified statements are
\texttt{flip\_chi\_circle\_bridge},
\texttt{net\_flip\_from\_chi\_lift}, and
\texttt{flip\_count\_from\_theta\_lifts}. Note that the zero-free
hypothesis is part of the verified statement: the alignment is proved
where the lifts are defined. The classical companion step --- that each
sign change of $\Re\zt$ records a zero of odd multiplicity on the line,
and that the winding step of the argument principle turns the flip count
into the count-with-multiplicity $N_0(T)$ of \eqref{eq:rvm} --- is not
part of the machine-verified content; it is the KNOWN winding argument of
Backlund \cite{titchmarsh}.

\emph{What is not claimed.} The two counting formulas \eqref{eq:rvm} and
the winding-to-count step of the argument principle (the thin rectangle
around the critical-line segment, Backlund; the folded rectangle of
Riemann--von~Mangoldt) are classical KNOWN mathematics
\cite{titchmarsh} and are not machine-verified in this paper. The
machine-verified content of this layer is exactly: the phase-lift
alignment of Theorem~\ref{thm:flipcount}, the completed-function
reflection of Theorem~\ref{thm:reflection}, the boundary estimates of
\S\ref{sec:boundary}, and --- the link between the two counts --- the
conditional equivalence $N_0(T)=N(T)$ under the Riemann hypothesis,
Theorem~\ref{thm:rheq}. An unconditional machine verification of the
assembly (that is, of \eqref{eq:rvm} for $N(T)$) is not claimed here.
...
$1/(2n+1)^x-1/(2n+2)^x$ are strictly positive
(\texttt{pairing\_term\_pos}), so the alternating series
$\eta(x)=\sum(2n-1)^{-x}-\sum(2n)^{-x}$ has positive partial sums and
positive limit: $\eta(x)>0$. The doubling identity
$\zt(x)=\eta(x)/(1-2^{1-x})$ and $1-2^{1-x}<0$ for $0<x<1$ give
$\zt(x)<0$. \qed

\emph{Euler--Maclaurin background.} On $(1,\infty)$ the Dirichlet series
shows $\zt>0$. The kernel $R_N(x)=\sum_{n\le N}n^{-x}-N^{1-x}/(1-x)$ is the
...
in \S\ref{sec:blockB}), and the limit $R(x)=\lim_N R_N(x)$ is the analytic
...
\]
since $x+1>1$. The paired series therefore converges absolutely, hence
...
other layer: the remaining thirteen layers are unchanged and remain
...
\texttt{symmetry\_cancel\_phase\_iso.lean} (iso~29 of the artifact, 10
...
\texttt{phase\_succ\_iso.lean}, iso~31, 4 declarations, 0 errors, 0
...
\texttt{continuum\_projection\_iso.lean}, iso~28 of the artifact: 15
...
comprises 231 declarations in \texttt{proof.lean} plus the isolation files
\texttt{zeta\_*\_iso.lean}, \texttt{continuum\_projection\_iso.lean}
(iso~28: the continuum by phase superposition, 35 declarations), and
\texttt{symmetry\_cancel\_phase\_iso.lean} (iso~29: what symmetry cancels
and the single-phase number field, 18 declarations), and
\texttt{path\_problem\_iso.lean} (iso~30: the path problem, 8
declarations), and \texttt{phase\_succ\_iso.lean} (iso~31: the
successor as phase superposition and only-0-and-1, 9 declarations), and
\texttt{zeta\_odd\_even\_continuity\_iso.lean} (iso~41: the odd/even
pairing continuity of this revision, 4 declarations); each isolation file
...
machine-verified through twelve layers: from the complex-axis representation
and the circle geometry, through the false-real-axis framework and the
critical-line phase layer, the covering-space phase locking, the power-law
error prototype, and the rectangle phase-lift bookkeeping and boundary
estimates (conditionally equivalent to the strip zero count,
Theorem~6.4). The negativity $\zt(x)<0$ on $(0,1)$ is closed by the odd/even
...
\appendix
\section{Complete declaration inventory (231 + 4 declarations, by layer)}
\label{app:inventory}

{\small\begin{sloppypar}
\begin{enumerate}
\item \textbf{L1 (11)}: \texttt{add}, \texttt{neg}, \texttt{zero},
...
\item \textbf{L2 (13)}: \texttt{orthogonal\_frame\_crt},
...
\item \textbf{L3 (22)}: \texttt{j}, \texttt{normSq}, \texttt{j\_sq},
...
\texttt{prime3Circle\_inter\_criticalCircle},
\texttt{primeCircle\_inter\_criticalCircle\_ge5}.
\item \textbf{L4 (27)}: \texttt{j}, \texttt{normSq}, \texttt{unitCircle},
...
\item \textbf{L5 (19)}: \texttt{complex\_mul\_imag\_leaks},
...
\texttt{euler\_plane}.
\item \textbf{L6 (9)}: \texttt{complex\_zeta\_convergence\_region},
\texttt{cosh\_lower\_bound}, \texttt{cosh\_unbounded},
...
\texttt{alternating\_partial\_sum\_bounded}, \texttt{eta\_pair\_form}.
\item \textbf{L8 (31)}: \texttt{zeta\_conj\_of\_one\_lt\_re},
...
\texttt{conj\_mellin\_f\_modif}, \texttt{conj\_completedRiemannZeta\_zero},
...
\texttt{orbit\_degenerate\_iff\_on\_line},
\texttt{recip\_on\_critical\_circle\_iff}, \texttt{zero\_orbit\_four}.
\item \textbf{L9 (36)}: \texttt{chi\_mul\_chi\_one\_sub},
...
\item \textbf{L10 (28)}: \texttt{zetaZeroSet}, \texttt{isClosed\_zetaZeroSet},
...
\item \textbf{L12 (12)}: \texttt{flip\_chi\_circle\_bridge},
\texttt{net\_flip\_from\_chi\_lift}, \texttt{flip\_count\_from\_theta\_lifts},
\texttt{completedZeta\_zero\_log\_reflection},
\texttt{zeta\_zero\_iff\_completedZeta\_zero\_one\_over\_sum},
...
\item \textbf{L13 (9, isolation file)}: \texttt{zetaEmR\_N},
...
\item The mechanical Lean~4 code for all 231 declarations and the
...
superposition), and the Euler--Maclaurin kernel is needed only for the sign
(\S\ref{sec:L7}, \S\ref{sec:kernel}, \S\ref{sec:blocksCG}).
...
must be assembled, not cited (\S\ref{sec:L8}, \S\ref{thm:bottom}).
...
in thirteen layers, plus the 4 declarations of the odd/even pairing of this
...
ZCode environment) performed the mechanical proof expansion, the writing of
